...
func TestIntegerArithmetic(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             "4 + 7",
			expectedConstants: []interface{}{4, 7},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
		...
		{
			input:             "2 * 4",
			expectedConstants: []interface{}{2, 4},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
...
func TestArrayLiterals(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             "[ 1 + 2, 3 - 4, 5 * 6, 14 / 7 ]",
			expectedConstants: []interface{}{1, 2, 3, 4, 5, 6, 14, 7},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpAdd),
...
func TestCompilerScopes(t *testing.T) {
	...
	if last.Opcode != code.OpAdd {
		t.Errorf("lastInstruction.Opcdep wrong. got=%d, want=%d", last.Opcode, code.OpAdd)
	}
...
func testConstants(
	...
	for i, constant := range expected {
		...
		case int:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testIntegerObjectfailed: %s", i, err)
			}
		case string:
