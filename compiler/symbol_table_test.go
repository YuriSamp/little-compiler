...
func TestResolveGlobal(t *testing.T) {
	...
	for _, sym := range expeced {
		...
		if result != sym {
			t.Errorf("expeced %s to resolve to %+v, got=%+v", sym.Name, sym, result)
		}
	}
}
...
func TestResolveLocal(t *testing.T) {
	...
	local := NewEnclosedSybmolTable(global)
	local.Define("c")
	local.Define("d")
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	expectedUnresolvvable := []string{
		"b",
		"d",
	}

	for _, name := range expectedUnresolvvable {
