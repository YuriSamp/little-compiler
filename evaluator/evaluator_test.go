...
func TestEvalBooleanExepression(t *testing.T) {
	tests := []struct {
		input string
		expected bool
	} {
...
func TestFunctionObject(t *testing.T){
	...
	if fn.Parameters[0].String() != "x" {
		t.Fatalf("parametes is not 'x', got%q", fn.Parameters[0])
	}
...
func TestStringConcatenation( t *testing.T) {
	...
	if str.Value != "Hello world honey!" {
		t.Errorf("String has wrong value. got=%q", str.Value)
	}
}

func TestBuiltInFucntions(t *testing.T) {
	tests := []struct {
		input string
		expected interface{}
	}{
